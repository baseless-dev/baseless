...
Deno.test("provider-auth-on-kv", async (t) => {
	await t.step("creates anonymous user", async () => {
	...
	await t.step("retrieve user with id", async () => {
		const kv = new SqliteKVProvider(":memory:");
		await kv.open();
	...
	await t.step("retrieve user with email", async () => {
		const kv = new SqliteKVProvider(":memory:");
		await kv.open();
	...
	await t.step("add sign-in with password", async () => {
		const kv = new SqliteKVProvider(":memory:");
		await kv.open();
	...
	await t.step("sign-in with email and password", async () => {
		const kv = new SqliteKVProvider(":memory:");
		await kv.open();
	...
	await t.step("validate email with code", async () => {
		const kv = new SqliteKVProvider(":memory:");
		await kv.open();
	...
	await t.step("reset password with code", async () => {
		const kv = new SqliteKVProvider(":memory:");
		await kv.open();
