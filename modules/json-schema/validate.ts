import { deepCompareStrict } from "./deep-compare-strict.ts";
	...
	if (Array.isArray($type)) {
		...
		if (!valid) {
			errors.push({
				...
				error: `Instance type "${instanceType}" is invalid. Expected "${
					$type.join(
						'", "',
					)
				}".`,
		...
		if (!stop && $patternProperties !== undefined) {
			...
			for (const pattern in $patternProperties) {
				...
				for (const key in instance) {
					...
					const subInstancePointer = `${instanceLocation}/${
						encodePointer(
							key,
						)
					}`;
			...
			if (length === 0 && $minContains === undefined) {
				...
				if (
					...
					errors.push({
						...
						keywordLocation: `${schemaLocation}/minContains`,
						error:
							`Array must contain at least ${$minContains} items matching schema. Only ${contained} items were found.`,
					});
