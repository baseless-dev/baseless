...
const build = new Command()
	...
	.option("--config <config:string>", "The Deno config file", {
		default: "./deno.json",
	})
	.option("--dist <dist:string>", "The distribution directory", {
		default: "./public",
	})
	...
	.action(async (options) => {
		...
		try {
			const denoJson = await Deno.readTextFile(
				join(Deno.cwd(), options.config),
			);
			const denoConfig = JSON.parse(denoJson) ?? {};
			if (
				"importMap" in denoConfig && typeof denoConfig.importMap === "string"
			) {
				try {
					const importMapPath = join(
						dirname(join(Deno.cwd(), options.config)),
					...
					for (const [url, map] of Object.entries(importMap?.imports ?? {})) {
						if (args.path.substring(0, url.length) === url) {
							args.path = join(
								importMapBase,
								map,
								args.path.substring(url.length),
							);
							break;
						}
					}
