...
// 	const next = getNextAuthenticationStepAtPath(auth.flow.tree, viewstate.flow);
...
authRouter.get("/", (request, _params, context) => {
	...
	return new Response(context.config.auth.renderer?.index(request, context), {
		status: 200,
		headers,
	});
});
...
authRouter.add(
	...
	async (request, params, context) => {
		...
		// 	return new Response(context.config.auth.renderer?.promptChoice({ request, step, isFirstStep, isLastStep: false, context }), { status: 200, headers });
		...
		// 			const counterKey = `/auth/challenge/${viewstate.id}/${step.type}/${slidingWindow}`;
		// 			if (await context.counter.increment(counterKey, 1, counterInterval) > step.sendLimit) {
		// 				logger.warn(`Authentication challenge ${step.type} for ${viewstate.id} rate limited.`);
		// 				headers.set("Retry-After", step.sendInterval.toString());
		// 				headers.set("Content-Type", "text/html; charset=utf-8");
		// 				return new Response(context.config.auth.renderer?.rateLimited(request, context), { status: 429, headers });
		// 			}
		// 			await step.send(request, context, viewstate.id);
		// 		}
		// 		if (step.prompt === "email") {
		// 			headers.set("Content-Type", "text/html; charset=utf-8");
		// 			return new Response(context.config.auth.renderer?.promptEmail({ request, step, isFirstStep, isLastStep, context }), { status: 200, headers });
		// 		} else if (step.prompt === "password") {
		// 			headers.set("Content-Type", "text/html; charset=utf-8");
		// 			return new Response(context.config.auth.renderer?.promptPassword({ request, step, isFirstStep, isLastStep, context }), { status: 200, headers });
		// 		} else if (step.prompt === "otp") {
		// 			headers.set("Content-Type", "text/html; charset=utf-8");
		// 			return new Response(context.config.auth.renderer?.promptOTP({ request, step, isFirstStep, isLastStep, context }), { status: 200, headers });
		...
		// 				logger.warn(`Authentication challenge ${step.type} for ${ip} rate limited.`);
		...
		// 				logger.log(`Authentication identification ${step.type} success.`);
		// 				viewstate.id = result;
		// 				viewstate.flow.push(step.type);
		// 				headers.set("Location", "/auth/login");
		// 				await saveViewState(headers, viewstate, context.config.auth.keys.algo, context.config.auth.keys.privateKey);
		// 				return new Response(null, { status: 301, headers });
		// 			} else {
		// 				logger.info(`Authentication identification ${step.type} failed.`);
...
// 	let viewstate: ViewState;
// 	try {
// 		viewstate = await deserializeViewState(cookies.viewstate?.toString() ?? "", auth.security.keys.publicKey);
// 	} catch (_err) {
// 		viewstate = { flow: [] };
// 	}
// 	return viewstate;
