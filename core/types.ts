...
export type First<Value extends unknown[]> = Value extends [infer Item, ...unknown[]] ? Item
	: never;

export type Last<Value extends unknown[]> = Value extends [...unknown[], infer Item] ? Item : never;

export type Split<Value extends string, Sep extends string> = Value extends
	`${infer A}${Sep}${infer B}` ? [A, ...Split<B, Sep>]
	: [Value];

export type UnionOmit<T, K extends string | number | symbol> = T extends unknown ? Omit<T, K>
	: never;
